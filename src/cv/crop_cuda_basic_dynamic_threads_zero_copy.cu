...
void CropCuda::crop_cuda_rgb_hwc_int8(const unsigned char *src, int src_width, int src_height,
    ...
    unsigned char *dev_src, *dev_dst;
    cudaHostGetDevicePointer(&dev_src, const_cast<unsigned char*>(src), 0);
    cudaHostGetDevicePointer(&dev_dst, const_cast<unsigned char*>(dst), 0);
    cudaEventRecord( stop, 0 );
    cudaEventSynchronize( stop );
    cudaEventElapsedTime( &elapsed_time, start, stop );
    printf("cudaHostGetDevicePointer cost %f ms\n", elapsed_time);
